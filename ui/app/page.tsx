...
export default function Home() {
  ...
  const { lastMessage: lastSseMessage, isConnected, connectionError } = useSse(sessionId, isAudio, sseEnabled, serverUrl);
  ...
  const { startAudio, stopAudio, isAudioStarted, playAudio } = useAudio(handleAudioData);
      ...
      if (lastSseMessage.mime_type === "text/plain") {
        ...
        setMessages((previousMessages) => {
          ...
          if (!last || last.role !== "assistant") {
            nextMessages.push({ id: generateId(), role: "assistant", content: chunk });
          } else {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    if (message.trim() !== "") {
      ...
      const assistantPlaceholder: ChatMessage = { id: generateId(), role: "assistant", content: "" };
      ...
      <div className="font-sans flex flex-col h-screen w-full overflow-hidden">
        <header className="sticky top-0 z-50 w-full flex items-center justify-between p-4 bg-black/30 backdrop-blur-xl border-b border-white/15">
          <div className="flex items-center gap-3 text-white rounded-2xl border border-white/20 bg-white/15 backdrop-blur-xl px-4 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_10px_30px_-10px_rgba(0,0,0,0.7)]">
            ...
            <span className="text-lg font-semibold text-white">Voice Agent</span>
          </div>
            ...
            {showSettings && (
              <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[90vw] p-3 sm:p-4 bg-black/85 backdrop-blur-xl border border-white/25 rounded-xl shadow-2xl z-50">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-white/80 block mb-1">Server URL</label>
                    <Input
                      value={tempUrl}
          ...
          <Card className="w-full max-w-4xl mx-auto flex-1 flex flex-col border-white/25 bg-white/12 backdrop-blur-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_20px_60px_-20px_rgba(0,0,0,0.7)]">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-white font-semibold">ADK Streaming Test</CardTitle>
                <div className="flex items-center gap-2">
                  <Badge variant={isConnected ? "secondary" : "destructive"} className={`backdrop-blur-md border text-white font-medium ${isConnected ? 'bg-green-500/20 border-green-400/40 text-green-100' : 'bg-red-500/20 border-red-400/40 text-red-100'}`}>
                    {isConnected ? "Connected" : "Disconnected"}
                  </Badge>
                  <Button type="button" size="sm" variant={isConnected ? "destructive" : "secondary"} onClick={handleToggleSse} className={`backdrop-blur-md border text-white font-medium h-8 ${isConnected ? 'bg-red-500/20 border-red-400/40 hover:bg-red-500/30 text-red-100' : 'bg-blue-500/20 border-blue-400/40 hover:bg-blue-500/30 text-blue-100'}`}>
              ...
              <form onSubmit={handleSendMessage} className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap justify-start">
                  ...
                  <Button 
                    type="button" 
                    variant="secondary" 
                    onClick={() => { setIsAudio(false); stopAudio(); }} 
                    disabled={!isAudioStarted} 
        ...
        <div className="px-2 sm:px-4 lg:px-6 pb-2 sm:pb-4 pt-2 sm:pt-4 flex-shrink-0">
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-1 sm:gap-2">
              <span aria-hidden="true" className="text-sm sm:text-lg text-white/90">
                ©
              </span>
              <span className="text-white whitespace-nowrap font-medium">Weights &amp; Biases</span>
            </div>
          </div>
          <div className="flex flex-row items-center gap-2 sm:gap-4">
            <div className="text-white/90 whitespace-nowrap">MIT License</div>
            <a
              href="https://github.com/wandb/voice-assistant-demo"
              target="_blank"
